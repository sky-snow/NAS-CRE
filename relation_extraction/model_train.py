...
def f_train(params):
    ...
    callback_history = model.fit(train_as_indices[:-1],
                                 ...
                                 epochs=200, batch_size=keras_models.model_params['batch_size'], verbose=1,
                                 validation_data=(
...
if __name__ == "__main__":
    ...
    #model.load_weights(args.models_folder + model_name + ".hdf5")
    ...
    if "train" in mode:
        ...
        callback_history = model.fit(train_as_indices[:-1],
                                     [train_y_properties_one_hot],
                                     epochs=100, batch_size=keras_models.model_params['batch_size'], verbose=1,
                                     validation_data=(
                                         val_as_indices[:-1], val_y_properties_one_hot),
                                     callbacks=[callbacks.EarlyStopping(monitor="val_loss", patience=5, verbose=1),
                                                callbacks.ModelCheckpoint(
                                                    args.models_folder + model_name + ".hdf5",
