...
def model_CNN(p, embedding_matrix, max_sent_len, n_out):
    ...
    pos_embeddings = layers.TimeDistributed(layers.Embedding(output_dim=p['position_emb'], input_dim=(max_sent_len*2)+1, input_length=max_sent_len,
...
def model_ContextWeighted(p, embedding_matrix, max_sent_len, n_out):
    ...
    # Take sentence encoded as indices and convert it to embeddings
    sentence_input = layers.Input(shape=(max_sent_len,),dtype='float32',name='sentence_input')
    print(sentence_input.shape)
    # sentence_input=sentence_input.set_shape((max_sent_len,))
    # print(sentence_input.shape)
    ...
    word_embeddings = layers.wrappers.TimeDistributed(layers.Embedding(output_dim=embedding_matrix.shape[1], input_dim=embedding_matrix.shape[0],
                                                                       input_length=max_sent_len, weights=[embedding_matrix],
                                                                       mask_zero=True, trainable=False))(x)
    #word_embeddings = layers.Dropout(p['dropout1'],input_shape=word_embeddings.shape)
    word_embeddings = tf.nn.dropout(word_embeddings,keep_prob=1-p['dropout1'])
    # Take token markers that identify entity positions, convert to position embeddings
    entity_markers = layers.Input(shape=(MAX_EDGES_PER_GRAPH, max_sent_len,), dtype='float32', name='entity_markers')
    pos_embeddings = layers.wrappers.TimeDistributed(layers.Embedding(output_dim=p['position_emb'],
                                                                      input_dim=POSITION_VOCAB_SIZE, input_length=max_sent_len,
                                                                      mask_zero=True, embeddings_regularizer = regularizers.l2(),
                                                                      trainable=True))(entity_markers)

    print("type",type(word_embeddings))
    print("type",type(pos_embeddings))
    # Merge word and position embeddings and apply the specified amount of RNN layers
    x=layers.Concatenate(axis=-1)([word_embeddings,pos_embeddings])
    #x = tf.concat([word_embeddings, pos_embeddings],axis=-1)
    print("concat",x.shape)
    for i in range(p["rnn1_layers"]-1):
        NASCell=tf.contrib.rnn.NASCell(p['units1'])
        lstm_layer=layers.RNN(NASCell,return_sequences=True)
        # lstm_layer = layers.LSTM(p['units1'], return_sequences=True)
        # if p['bidirectional']:
        #     lstm_layer = layers.Bidirectional(lstm_layer)
        x = layers.TimeDistributed(lstm_layer)(x)
    #lstm_layer = layers.LSTM(p['units1'], return_sequences=False)
    NASCell = tf.contrib.rnn.NASCell(p['units1'])
    lstm_layer = layers.RNN(NASCell, return_sequences=False)
    # if p['bidirectional']:
    #     lstm_layer = layers.Bidirectional(lstm_layer)
    sentence_matrix = layers.TimeDistributed(lstm_layer)(x)
    ...
    for i in range(MAX_EDGES_PER_GRAPH):
        ...
        sentence_scores = layers.Lambda(lambda inputs: K.batch_dot(inputs[0], inputs[1], axes=(1, 2)),
    ...
    # Apply softmax
    edge_vectors = layers.Dropout(p['dropout1'])(edge_vectors)
    main_output = layers.TimeDistributed(layers.Dense(n_out, activation="softmax", name='main_output'))(edge_vectors)

    model = models.Model(inputs=[sentence_input, entity_markers], outputs=[main_output])
    #optimizer = optimizers.Adam(lr=p['lr'],decay=0.0001)
    optimizer=tf.train.AdamOptimizer(learning_rate=p['lr'])
    #optimizer1=optimizers.SGD(lr=0.0001,decay=0.0003,momentum=0.9,nesterov=True)
...
class MaskedGlobalMaxPooling1D(layers.GlobalMaxPooling1D):
    def __init__(self, **kwargs):
        self.supports_masking = True
